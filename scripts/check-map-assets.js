...
if (!/^override REPO_ROOT := \$\(shell path=/m.test(makefile) || !makefile.includes('/usr/bin/dirname') || !makefile.includes('/bin/pwd -P')) {
...
for (const command of [
  ...
  if (!lintTarget || !lintTarget[1].includes(`\t${command}\n`)) {
    fail(`Makefile lint target must run from the repository root: ${command}`);
  }
}
...
for (const contract of ['Power Line Map', '18 target/override cases', '2 MAKEFILE_LIST rejection cases', 'MAKEFILE_LIST must not be overridden']) {
...
for (const evidence of ['Status: Completed', 'five pre-existing public Make targets plus the root regression gate', '18 target and `REPO_ROOT` override cases', 'Command-line and environment `MAKEFILE_LIST` overrides failed closed']) {
